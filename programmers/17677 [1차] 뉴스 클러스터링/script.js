...
//다중집합 제작기
function toMulti(s,v) {         //들어오면 2개씩 자르고 
    let multiJ=[]
...
function Exercise_17677(str1, str2) {
    var answer = 0;
    let count = 0;
    ...
    if (JAs>=JBs) {                         //둘 중 보다 많은 다중집합을 기준으로 진행한다.
        for(i=0; i<=JAs; i++) {
        if(JB.indexOf(JA[i])!=-1){                      //JA[i]가 JB에 있으면
            console.log(JA[i],'는 교집합의 원소입니다.')   //JA[i]는 교집합의 원소이다.
            JB.splice(JB.indexOf(JA[i]),1)      //JB에서 첫번째 JA[i]와 같은 원소 제거
            count++     //교집합의 크기
           }
        } 
    }
        ...
        for(i=0; i<=JBs; i++){
            if(JA.indexOf(JB[i])!=-1){
            console.log(JB[i],'는 교집합의 원소입니다.')
            JA.splice(JA.indexOf(JB[i]),1)
            count++                        
           }
        }
    }
    
    if (count == 0 && (JAs+JBs-count) ==0) {
        answer = 65536
    }
    else answer = Math.floor(count/(JAs+JBs-count) *65536)
...
runResult(Exercise_17677(ex_str1,ex_str2),ex_Ans)